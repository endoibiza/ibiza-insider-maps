...
- Ibiza Rocks queue workflow: for official recurring pages that publish only concept copy such as Bingo Brunch or generic resident-DJ/live-musician pool-party text, upsert `event_source_links` with `status = needs_review` and `raw_metadata.queued_reason = official_series_page_has_generic_or_resident_dj_copy_only`. This creates an explicit queued exception without polluting public `lineup_details`.
