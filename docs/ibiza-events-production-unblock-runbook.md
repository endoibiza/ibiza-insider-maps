...
- Night League fullness workflow: non-empty lineups are not automatically complete. Treat headliner-only rows such as `ANYMA`, `John Summit`, `David Guetta`, `Martin Garrix`, `Calvin Harris`, or `Swedish House Mafia` as freshness defects when official UNVRS/Ushuaïa/Hï sources publish support artists. Use `scripts/night-league-lineup-fullness.mjs` for the daily cloud report and for staging exact official-source proposals; publish those proposals only through the guarded `event-lineup-apply-safe-batch` workflow with exact-date refresh enabled.
- Playa Soleil workflow: do not add or force a Playa Soleil venue chip unless visible Supabase rows or reviewed event candidates exist under the official venue name. The Night League fullness report checks the official Playa Soleil site for discovery signals, but real events should still be staged/deduped before public rows are created.
...
- Fourvenues public-page rule: public Fourvenues event/iframe pages may be used as non-monetized evidence for date/title/source checks before Channel Manager access exists. Do not treat those public links as the future monetized checkout integration. Monetized Fourvenues rows/booking fields still wait for the Channel Manager API key and venue approvals.
- 2026-06-23 Fourvenues portal update: after Inacio enabled API access, `Settings > Developer Portal` appeared for organization `Ibiza Maps` (`ibiza-maps`). A Channel Manager API request named `Ibiza Maps Channel Manager API - Supabase sync` was submitted with `all:read`, expiry `23/06/2027`, and status `Requested`. No API key was exposed yet, so `FOURVENUES_API_KEY` remains unset. Next step after Fourvenues approves/issues the key: store it as the Supabase secret `FOURVENUES_API_KEY`, run `sync-fourvenues-auth`, inspect approved channel/host organizations, then run a narrow `sync-fourvenues-events` dry run before public writes.
- Pikes exact-page workflow: `https://www.pikesibiza.com/whats-on/` gives exact event URLs, but the real lineup is on each `/event/.../` page inside the `event-specs` `LINE-UP` block. Reject prose, footer/menu noise, `Secret DJs`, `Very Special Guests`, `Pikes Resident DJs`, `Coming Soon`, `& more`, and partial announcements as public lineup replacements. Record exact-page checks in `event_source_links`; apply only when the exact page exposes a clean named-artist list.
- Cova Santa / Fourvenues iframe workflow: `https://www.covasanta.com/en/events#events` embeds Fourvenues. `web.fourvenues.com/en/iframe/cova-santa/events?date=YYYY-MM` exposes static event cards with exact slugs/codes/dates, and some exact iframe event URLs open locally, but static payloads do not currently expose clean weekly artist lineups. Queue with `cova_santa_fourvenues_page_blocked_or_no_static_lineup` and route artist extraction to cloud browser/OpenClaw or Channel Manager API.
- Ibiza Rocks recurring-page workflow: official recurring pages for Nothing New, Ibiza Rocks Pool Party, and Bingo Brunch publish concept/resident copy, not date-specific artist lineups. Keep them queued with `official_series_page_has_generic_or_resident_dj_copy_only`. Ibiza Anthems is a good recurring exception where the official page names residents; current rows use `Switch Disco, Chris Watson, Ellie Sax`.
...
- Pikes structured-data workflow: `https://www.pikesibiza.com/whats-on/` embeds JSON-LD `Event` objects with date-specific event URLs and descriptions. Treat this as the cheapest official feed before browser crawling. Reject descriptions containing `SECRET DJS`, `VERY SPECIAL GUESTS`, `SPECIAL GUEST`, `RESIDENT DJS`, `COMING SOON`, `TBA`, ellipses, or `& MORE`; apply only exact named-artist deltas. Example: 528 row for Pikes Presents on 2026-06-30 was safely expanded to `Folamour, Melon Bomb, Josh Wink, Antonio Prosper, Marlon Lopez` while preserving its Fourvenues ticket URL.
...
- Repair exception workflow: use private table `event_repair_exceptions` for rows that cannot honestly be represented in `event_source_links`, especially missing-URL rows where no source URL is known yet. The table has RLS enabled and no anon/authenticated client access. Use `issue_type = missing_event_url` with queue reasons such as `manual_local_event_needs_official_program_url`, `local_event_x_source_needs_official_url_discovery`, and `tomodachi_missing_shotgun_url_requires_browser_verification`.
...
- DICE slug date workflow: DICE event URLs often encode dates as `25th-jun`, `2nd-jul`, etc. Treat these as exact-date evidence only when the Supabase row date/year and event title also match the DICE event title. The safe apply script now recognizes day-month slugs without a year; use the standard workflow first before manual SQL. Solid Grooves rows can be safely updated from DICE JSON-LD performer lists when the proposal contains named artists and not a partial/empty extraction.
- UNVRS official URL workflow: the official `unvrs.com/events-calendar` page can expose JSON-LD `MusicEvent` objects with exact date-specific URLs even when the card still says `Artists TBA`. It is safe to replace third-party/generic URLs with those exact official URLs and mark source links as `needs_review` with `queued_reason = unvrs_official_page_confirms_event_but_artists_tba`; do not invent artists when the JSON-LD performer list is empty.
- Cova Santa public Fourvenues calendar workflow: `web.fourvenues.com/en/iframe/cova-santa/events?date=YYYY-MM` exposes official public event cards for far more Cova dates than the old Notion import captured. Stage non-obvious cards first in `event_candidates` with `source_key = cova-santa-fourvenues-public-calendar`, `source_url_type = fourvenues_public`, and `maintenance_flags` such as `new_cova_santa_official_candidate` or `possible_cova_santa_date_replacement`. Do not publish these directly: some dates conflict with old rows (for example old Pantheøn rows where the current official card says Levi Special Edition), so they need review/dedupe before public inserts or replacements.
- Fourvenues organization/API note: Inacio confirmed the Channel Manager API key must be requested by an admin user inside the Fourvenues organization under Settings > Developer Portal. If the Developer Portal is missing, either fiscal information is incomplete or the organization is inactive; send Fourvenues the org name/slug so they can check activation. Event updates have no webhook, so the production design should use polling/reconciliation after venue collaborations are approved.
- 2026-06-22 production repair update: Cova Santa public Fourvenues calendar candidates can be promoted in small reviewed batches when there is no same-date/name public row. Use stable `agent:cova-santa-fourvenues-public:{external_id}` IDs, exact public Fourvenues event URLs, `source_type = fourvenues_public`, and conservative public copy such as `{event} at Cova Santa.` when weekly artists are not exposed. Do not merge conflicting dates automatically; keep them in `needs_review`.
- 2026-06-22 Pikes update: `/whats-on/` JSON-LD is useful for finding exact event URLs, but individual `/event/.../` pages are the reliable lineup source. Prefer the visible `LINE-UP ... How to be there` block over truncated JSON-LD. Publish only clean named lists; record source checks and queue pages with `Secret DJs`, `Very Special Guests`, `Pikes Resident DJs`, `Coming Soon`, `Special Guest`, `& more`, or ellipses.
- 2026-06-22 Amnesia update: CM.com ticket pages may expose partial official metadata. `Dreamteam, La Movida, + more TBA` is an honest partial lineup for Rememberland and can be represented publicly as `Dreamteam, La Movida (+ more TBA)` when the alternative is stale generic TBA. Keep it counted/queued as partial, not complete.
- 2026-06-22 Circoloco update: live Supabase rows were verified as Mondays with full lineups. If the site appears to show Tuesday, check frontend grouping/timezone/display before changing data; do not date-shift rows unless official Circoloco/DC10 evidence conflicts with the database.
- 2026-06-22 Ushuaïa update: rows with named artists and official Ushuaïa event URLs may have stale `status = TBA`. A status-only repair to `Confirmed` is safe when lineup text is non-empty and does not contain TBA/coming-soon/unannounced language. Do not change lineups or URLs during that status repair.
- 2026-06-22 Eden update: official Eden residency pages such as `/galactica/` and `/markus-schulz/` return live pages and embed `custom-iframe.fourvenues.com`, but do not expose weekly lineups in static HTML. `Markus Schulz (all night long)` rows can be status-confirmed from the official page; Galactica rows should remain queued with `eden_galactica_iframe_no_static_weekly_lineup` until cloud browser/OpenClaw or Fourvenues Channel Manager exposes weekly artists.
- 2026-06-22 Tomodachi update: local Shotgun fetches for event pages, venue pages, and search return Vercel Security Checkpoint 429. Existing exact Shotgun URLs remain valid source evidence, but do not guess missing future slugs. Queue rows with `tomodachi_shotgun_tba_or_browser_required`; for missing URLs use `tomodachi_missing_shotgun_url_requires_browser_verification`.
- 2026-06-22 Other/municipal missing URL workflow: many `venue = Other` rows are manual annual/local fixtures where a stable public row is useful but the exact 2026 official programme URL is not published yet. Do not fill these with generic homepages just to reduce the missing-URL count. Use private `event_repair_exceptions` with `issue_type = missing_event_url` and queue reasons such as `annual_local_fiesta_needs_official_program_url`, `future_program_or_dates_not_published`, `future_festival_program_not_published`, `sports_registration_url_not_yet_verified`, `walking_ibiza_exact_event_url_needed`, or `local_event_x_source_needs_official_url_discovery`.
- 2026-06-22 queue hygiene workflow: when a newer, more specific queued reason is added for a visible event, dismiss older active repair exceptions for the same issue so final audits count one active reason per event/issue. Verify with a duplicate-active-exception query before reporting final queue totals.
- 2026-06-22 live spot-check workflow: after public batches, check representative live event detail URLs on `https://ibiza-maps.com/events/{slug}` and pair HTTP route checks with Supabase slug existence. A `200` confirms the deployed route is reachable, but visual/browser QA is still needed when the concern is client-side rendering or timezone grouping.
- 2026-06-22 Club Chinois / Fourvenues iframe workflow: the Defected and La Troya residency pages embed public Fourvenues iframe groups that expose exact weekly lineups and event links in static HTML. `chinois-ibiza@g:dqjdq` covers Defected; `chinois-ibiza@g:nhwoc` covers La Troya. As of the 2026-06-22 audit, all `63` visible upcoming Club Chinois rows have event URLs, no weak/TBA lineups, and no generic/partial lineup flags. Use this as the green-path pattern for other Fourvenues iframe venues.
- 2026-06-22 Amnesia calendar workflow: the official Amnesia all-season calendar can expose fuller row-level lineup text than individual CM.com ticket pages. Always inspect the `<article>` card around the matching ticket link before accepting a partial ticket-page extraction. For Rememberland, later 2026 rows still visibly show `DREAMTEAM / LA MOVIDA / + TBA`, so keep those partial and queued rather than overstating them. For Bresh, rows that show `SPECIAL GUEST TBA` remain queued until a named guest appears.
- 2026-06-22 queue-completeness audit: before reporting production readiness, run the unqueued-defect query across visible upcoming rows. A row is a defect if it has a missing URL, missing lineup, weak/TBA lineup, generic/partial lineup, or `status = TBA`. If the row cannot be safely repaired from official evidence, add a private queue reason in `event_source_links` or `event_repair_exceptions`. The 2026-06-22 pass reached `0` visible defect rows without an explicit queue reason.
- 2026-06-23 public-facing audit: using the anon/public Supabase surface, `/events` currently has `1795` visible upcoming rows, `31` missing URLs, `684` Spotlight URLs, `586` official venue URLs, `19` generic URLs, `27` missing lineups, `124` weak/TBA lineups, `105` generic/partial lineups, `98` rows still marked `TBA`, and `0` visible Fourvenues-owned rows. Private queue coverage could not be refreshed without the service-role key, so treat these as public defect counts, not unqueued-defect counts.
- 2026-06-23 live route QA: 10 representative event detail pages returned `200` on `https://ibiza-maps.com/events/{slug}` across Pikes, Cova Santa, UNVRS, Hï, 528, Club Chinois, Amnesia, Circoloco, Other/local, and Ibiza Rocks. Circoloco database dates are Mondays; some preserved slugs still contain the old Tuesday date token, so public display should use the `date` field rather than slug text.
- 2026-06-23 next freshness targets while waiting for the Fourvenues API key: prioritize public-source packets for Pikes (`88` public issue rows), Cova Santa (`44`), Ibiza Rocks (`34`), Other/local (`33`), 528 (`12`), Tomodachi (`11`), Amnesia (`11`), Eden (`10`), DC10 (`7`), Pacha (`4`), UNVRS (`3`), and Ushuaïa (`2`). Do not run broad crawls first; use existing exact URLs and venue-specific parsers.
- 2026-06-23 partial-lineup apply mode: the safe lineup apply workflow now has an opt-in `allow_partial_source_backed` flag. Use it only when an exact official/trusted source exposes real named artists plus a partial marker (`& more`, `+ TBA`) and the current public row is blank/TBA/Coming Soon. Public formatting must be explicit, e.g. `Cinthie, Laura & Santiago (+ more TBA)`, so the row improves customer-facing accuracy without pretending the lineup is complete.
- 2026-06-24 Fourvenues Channel Manager key status: the one-time key was stored as Supabase secret `FOURVENUES_API_KEY`. `sync-fourvenues-auth` succeeded and returned channel `Ibiza Maps` / slug `ibiza-maps`, but `host_count = 0`; 7-day and 180-day `sync-fourvenues-events` dry runs returned `records_seen = 0`. Interpretation: API plumbing is working, but no venue/host collaborations are attached yet, so Channel Manager cannot return monetizable venue events.
- 2026-06-24 Fourvenues collaboration workflow: Fourvenues dashboard path is `pro.fourvenues.com/ibiza-maps/settings/hosts` (`Settings > Partnerships`). It shows `No collaborations yet` and `Find collaboration`. Searching for a venue such as `Pacha` exposes candidate organizations including `Pacha Ibiza`; selecting a venue opens `Request collaboration` with `transparent` vs `opaque` collaboration. Do not submit requests without Mike's exact approval. Based on the docs/thread, venue acceptance is required before events are automatically published/synced for Ibiza Maps.
- 2026-06-24 Fourvenues thread evidence: Alvaro confirmed Channel Manager is the preferred route, Fourvenues can create/list a marketplace organization, venues discover/request or approve collaborations, and once accepted events/updates publish automatically; commercial terms are agreed with each venue. Inacio confirmed API keys are requested under `Settings > Developer Portal`, fiscal info/org activation are prerequisites, venues must invite/approve collaboration, and there are no event-created/event-updated webhooks, so use polling/reconciliation or dynamic reads.
- 2026-06-24 Fourvenues profile status: `Settings` still shows the generic venue onboarding checklist as `2/5 steps completed` (`Create your first event`, `Set up a service`, and `Add your tax details` remain listed), but the tax details page is populated and locked with instructions to email `help@fourvenues.com` for billing updates. Do not create dummy events or services unless Fourvenues confirms they are required for a marketplace/channel organization. The useful profile improvements before collaboration requests are adding real organisation SEO/profile copy, optional social links, and better logo/cover assets; these are trust/marketplace polish, not Channel Manager API blockers.
- 2026-06-24 Fourvenues profile polish: SEO/profile copy was filled in through the dashboard for Ibiza Maps as an island discovery and distribution channel, not a venue. Use Michael's real Ibiza beach photos for profile artwork, not generated/stock-looking images. Final cover options are in `/Users/michaelhenderson/Desktop/Ibiza Maps Cover/finished/`; the recommended/current cover is `ibiza-maps-fourvenues-cover-real-beach-1080x1350.jpg`, also copied to `tmp/fourvenues-profile/ibiza-maps-fourvenues-cover-1080x1350.jpg`. A temporary square logo is at `tmp/fourvenues-profile/ibiza-maps-fourvenues-logo-400x400.png`. Chrome blocked automated file upload until the Codex extension has local file URL access, so upload these manually or retry after enabling that permission.
- 2026-06-24 event quality sprint: latest cloud repair report shows `1778` visible upcoming rows, `27` missing lineups, `123` weak/TBA lineups, `104` generic lineups, `28` missing URLs, `682` Spotlight URLs, `363` ticketing URLs, `580` official URLs, and `0` date-mismatched event URLs. Pikes remains the largest club issue cluster (`88` issue rows: `27` missing lineups, `29` weak/TBA, `60` generic). A 60-day/30-row GitHub Actions Pikes browser packet completed successfully with `30` targets checked, `30` snapshots inserted, `0` proposals, `0` source failures, `0` public inserts, and `0` public updates. Interpretation: the generic Playwright runner can render exact Pikes pages and record evidence, but current near-term exact pages did not expose clean new named-artist deltas beyond existing/generic queued text. Do not keep rerunning broad Pikes packets; use exact URL batches or a Pikes-specific parser/report to classify the remaining queued pages.
- 2026-06-24 Cova Santa sprint routing: a generic Cova Santa GitHub Actions browser packet (`window_days=180`, `limit=50`, `venue_pattern=Cova Santa`) did not finish within the useful threshold and was cancelled before public writes. Route Cova Santa to an exact-source/OpenClaw parser path instead of venue-pattern broad sweeps: start from `covasanta.com/en/events` and `web.fourvenues.com/en/iframe/cova-santa/events?date=YYYY-MM`, stage exact public Fourvenues event URLs/candidates, and only propose lineups when named weekly artists are actually visible.
- 2026-06-26 Chinois Channel Manager workflow: Chinois is approved and visible through Fourvenues auth as host `wlw4t6xtf013101dd1sstd9pw5199tkW`. A 180-day sync returned `95` official records, `0` ticket-rate rows, `0` guest-list rows, and `10` VIP/table rows. The API event `name` often contains the full artist list while `artists`/`lineup_details` can be partial or generic, so the Chinois reconciler extracts comma-separated artist lists from the official event title, preserves fuller verified legacy lineups, copies Mike's Picks/editorial flags, copies old official/manual links as fallback options only when no Fourvenues ticket rates exist, and hides legacy duplicates after the official row is safe. Do not label Chinois iframe links as `Tickets` until Channel Manager exposes ticket rates or Fourvenues/Chinois confirms the iframe URL is the commissionable ticket checkout surface.
- 2026-06-26 booking-options correction: Public Chinois rows are all Fourvenues-owned with no visible legacy duplicates, but the currently synced Chinois host still exposes `0` ticket options. Treat Chinois ticket inventory as blocked until every Chinois-like approved host is dry-run compared or the venue/Fourvenues identifies which host owns ticket rates. Use `scripts/fourvenues-host-dry-run-audit.mjs` via the `fourvenues-host-dry-run-audit` workflow to compare duplicate venue approvals before choosing a host ID. Use `scripts/event-booking-options-audit.mjs` after every booking repair. The cleanup policy is: `Tickets` only for ticket rates or verified ticketing URLs, `VIP / Tables` only for booking/table availability, `Official Info` for official venue pages, and `More Info` only when no stronger option exists.
